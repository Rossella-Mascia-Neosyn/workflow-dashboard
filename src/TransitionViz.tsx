...
import { Transition } from './myUtils/model/MoreComplexData';
...
const getDelayFromEventType = (
  ...
  try {
    ...
    if (Number.isNaN(finalDelay)) {
      const delayExpr = delayOptions && delayOptions[delay];
    ...
    >
      <div data-viz="transition-label">
        <span data-viz="transition-event">
          <EventTypeViz actions={definition.actions} internal={definition.internal} eventType={definition.eventType} delay={delay} />
        ...
        {definition.actions.length > 0 && (
          <div data-viz="transition-actions">
            {definition.actions.map((action, index) => {
              return (
                <PopoverCustom action={action as Transition} key={index}>
                  <ActionViz action={action} kind="actions" />
                </PopoverCustom>
              )
            })}
          </div>
