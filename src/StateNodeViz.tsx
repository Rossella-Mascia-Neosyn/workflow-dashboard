...
import { EntryAction } from './myUtils/model/MoreComplexData';
...
const StateNodeKey: React.FC<{ value: string, id: string, version: any }> = ({ value, id, version }) => {
  return (
    <div data-viz="stateNode-key">
      {
        !id.includes('.') ?
          <Flex maxWidth="none" justify="space-between" data-viz="stateNode-keyText" title={value}>
            <div>
              {value}
            </div>
            <div>
...
export const StateNodeViz: React.FC<{
  ...
  return (
    <div
      ...
      style={{
        outline: '2px solid var(--unicredit-primary-color-hover)',
        position: 'absolute',
      ...
      >
        ...
        >
          <div data-viz="stateNode-header">
            ...
            <StateNodeKey id={stateNode?.id} value={stateNode.key} version={stateNode?.version} />
          ...
          {stateNode.definition.entry.length > 0 && (
            <div data-viz="stateNode-actions" data-viz-actions="entry">
              {stateNode.definition.entry.map((action, index) => {
                return (
                  <PopoverCustom action={action as EntryAction} key={index} >
                    <ActionViz action={action} kind="entry" />
                  </PopoverCustom>
                )
              })}
            </div>
          ...
          {stateNode.definition.exit.length > 0 && (
            <>
              <div data-viz="stateNode-actions" data-viz-actions="exit">
                <div data-viz="action-kind">exit</div>
                <div>
