...
export const EventTypeViz: React.FC<{
...
}> = ({ eventType: event, delay, onChangeEventType, internal, actions }) => {
  ...
  return (
    <div data-viz="eventType">
      <span data-viz="eventType-text">{event}</span> <span data-viz={actions.length > 0 ? "action-type-internal" : "no-action-type"}>{internal ? 'Internal' : 'External'}</span>
    </div>
  );
};
